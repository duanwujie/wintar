...
void data_extract_all(archive_handle_t *archive_handle)
{
	...
	if (archive_handle->ah_flags & ARCHIVE_UNLINK_OLD) {
		...
		struct port_stat existing_sb;
		if (port_stat(file_header->name, &existing_sb) == -1) {
			if (errno != ENOENT) {
	...
	case S_IFDIR:
#if _WIN32
		res = port_mkdir(file_header->name);
#else
		res = port_mkdir(file_header->name, file_header->mode);
#endif
		if ((res == -1)
